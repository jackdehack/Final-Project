...
public class CollisionObj {
	...
	private Vector lineProjDist(double x1, double y1, double x2, double y2) {
		...
		Vector linetoBall = new Vector((ballX - x1), (ballY - y1)); // vector from the first point on the rect to the
																	// ball's center
		Vector lineVector = new Vector((x2 - x1), (y2 - y1)); // vector of the rectangle side length

		double lineLengthSquared = lineVector.x * lineVector.x + lineVector.y * lineVector.y; // scalar for the dot
																								// product
		double projection = linetoBall.dotProduct(lineVector) / lineLengthSquared; // gives us a number of how alligned
																					// the ball line is with the
																					// rectangle wall
	...
	protected boolean checkCollides(CollisionObj other) {
		if (other.rectHitBox) {
			...
			for (int i = 0; i < wall.points.length; i++) {
				...
				Point2D p2 = wall.points[(i + 1) % wall.points.length]; // goes around rectangle points, and wraps
																		// around for the last line
